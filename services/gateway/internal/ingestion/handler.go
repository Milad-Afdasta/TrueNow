...
type Handler struct {
	...
	metricsRoute fasthttp.RequestHandler
...
func NewHandler(v *validator.SIMDValidator, p *producer.BatchProducer, r *ratelimit.HierarchicalLimiter, m *metrics.PrometheusExporter) *Handler {
	h := &Handler{
		validator:   v,
		producer:    p,
		rateLimiter: r,
	...
	if m != nil {
		h.metricsRoute = fasthttpadaptor.NewFastHTTPHandler(m)
	}
...
func (h *Handler) Handle(ctx *fasthttp.RequestCtx) {
	...
	defer func() {
		if h.metrics != nil {
			h.metrics.RecordRequest(time.Since(start), success)
		}
	}()
	...
	if !bytesEqual(path, []byte("/v1/ingest")) {
		...
		if h.metrics != nil {
			h.metrics.RecordError("route_not_found")
		}
		return
	}
	...
	if len(namespace) == 0 || len(table) == 0 {
		...
		if h.metrics != nil {
			h.metrics.RecordError("missing_namespace_table")
		}
		return
	}
	...
	if !h.rateLimiter.Allow(nsStr) {
		...
		h.requestsRejected.Add(1)
		if h.metrics != nil {
			h.metrics.RecordError("rate_limited")
		}
		return
	}
	...
	body := ctx.PostBody()
	bodySize := len(body)
	h.bytesProcessed.Add(uint64(bodySize))
	...
	if len(events) == 0 {
		...
		if event == nil {
			...
			h.requestsRejected.Add(1)
			if h.metrics != nil {
				h.metrics.RecordError("invalid_event")
			}
			return
		}
	...
	if len(batch) == 0 {
		...
		h.requestsRejected.Add(1)
		if h.metrics != nil {
			h.metrics.RecordError("no_valid_events")
		}
		return
	}
	...
	if err != nil {
		...
		h.requestsRejected.Add(1)
		if h.metrics != nil {
			h.metrics.RecordError("kafka_send_failed")
		}
	...
	if h.metrics != nil {
		h.metrics.RecordEvents(int64(len(batch)), int64(bodySize))
	}
	success = true
...
func (h *Handler) handleNonPost(ctx *fasthttp.RequestCtx) bool {
	...
	case "/health":
		...
		return true
	case "/metrics":
		if h.metricsRoute == nil {
			ctx.SetStatusCode(fasthttp.StatusServiceUnavailable)
			ctx.SetBodyString("metrics exporter not configured")
			if h.metrics != nil {
				h.metrics.RecordError("metrics_unavailable")
			}
			return false
		}
		h.metricsRoute(ctx)
