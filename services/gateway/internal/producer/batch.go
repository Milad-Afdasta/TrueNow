...
func NewBatchProducer(brokers []string, metricsExporter *metrics.PrometheusExporter) *BatchProducer {
...
func (bp *BatchProducer) SendBatch(events []*Event) ([]int64, error) {
	...
	if bp.writers == nil {
		...
		if bp.metrics != nil {
			bp.metrics.RecordKafkaMetrics(int64(len(events)), 0, 0)
		}
		return offsets, nil
	}
	...
	if err != nil {
		...
		if bp.metrics != nil {
			bp.metrics.RecordKafkaMetrics(0, int64(len(messages)), 0)
			bp.metrics.RecordError("kafka_send_failed")
		}
		return nil, err
	}
	...
	if bp.metrics != nil {
		bp.metrics.RecordKafkaMetrics(int64(len(messages)), 0, 0)
	}
